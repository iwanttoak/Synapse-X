...
> **状态**: v3 — GPU 预处理已交付 (NVRTC)。170 FPS 稳态，LIFO 零积压，全链路 ~1.7 ms。
...
| MSVC 工具链 | 14.51 (v180，用于 C++ 代码) | 自动检测 |
| MSVC 工具链 | 14.44 (v143，用于 NVRTC 主机编译器) | 自动检测 |
...
Producer (main thread, core 0):       Consumer (worker, core 1):
  ...
  Per-second stats + BMP                Warmup 50 frames
                                        while g_running:
...
- **绑核**: `SetThreadAffinityMask`，生产者=核心0，消费者=核心1

### 3.2 通过 NVRTC 进行 GPU 预处理 (v3)
...
**性能**：
- GPU 执行时间: ~15–30 μs（内存带宽受限，非计算受限）
...
| 字段 | 含义 |
|-------|---------|
| `recv` | UDP + 重组 + LZ4 解压（生产者） |
| `infer` | cudaMemcpyAsync + GPU kernel + TRT + sync + 后处理（消费者） |
